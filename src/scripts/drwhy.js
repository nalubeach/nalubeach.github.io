  document.addEventListener('DOMContentLoaded', () => {
    // menu, modais, popups — se necessário, ou mantê-los no main.js

    const form = document.getElementById('quizForm');
    if (form) setupDrWhyForm(); // configura apenas se existir
  });
  ...
  function setupDrWhyForm() {
    ...
    function validate() {
      let ok = true;
      if (!teamName.value.trim()) { teamName.style.borderColor = '#c15423'; ok = false; }
      else teamName.style.borderColor = '#5ca096';
      if (team.checked && !numPlayers.value) { numPlayers.style.borderColor = '#c15423'; ok = false; }
      else if (team.checked) numPlayers.style.borderColor = '#5ca096';
      return ok;
    }
    ...
    form.addEventListener('submit', function (e) {
      ...
      fetch(form.action, { method: 'POST', body: fd })
        .then(r => r.json())
        .then(json => {
          if (json.success) showSuccess(teamName.value.trim(), numPlayers.value, team.checked);
          else alert('Erro interno, tente novamente.');
        })
        .catch(err => {
          console.error(err);
          alert('Erro ao submeter formulário: ver console.');
        })
        .finally(() => {
